(function(){
  ...
  fetch("/api/me",{credentials:"same-origin"}).then(r=>r.json()).then(data=>{const u=data.user||{};const id=u.company_id||data.activeCompanyId||"";const link=panel.querySelector("#afa-contact");link.href=`https://wa.me/972559665585?text=${encodeURIComponent(`AI FLOW Support\nAccount: ${u.email||""}\nClient ID: ${id||"Platform Admin"}\nPage: ${location.pathname}\n\nHow can we help?`)}`}).catch(()=>{});
})();
